...
export function StudentLocationMap({ studentData }: StudentLocationMapProps) {
  ...
  useEffect(() => {
    ...
    const defaultLat = studentData?.latitude || 20.5937
    const defaultLng = studentData?.longitude || 78.9629
    ...
    try {
      ...
      // If student has location data, show it
      if (studentData?.latitude && studentData?.longitude) {
        ...
        addMarker(map, studentData.latitude, studentData.longitude)
        setLoading(false)
      } else {
        // Auto-start tracking if no student data
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        if (err.code === 1) {
          ...
          errorMsg = 'Location unavailable. Please ensure GPS/location services are enabled.'
        } else if (err.code === 3) {
  ...
  const updateLocationOnBackend = async (latitude: number, longitude: number, accuracy?: number) => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/student/update-location', {
        method: 'POST',
        headers: {
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>Your current GPS location visible to nearby vendors</CardDescription>
          </div>
          <button
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <p className="text-sm text-yellow-800">
            <span className="font-semibold">📍 Note:</span> Your location will be shared with nearby vendors to help them understand customer locations. This improves service personalization.
          </p>
        </div>
      </CardContent>
    </Card>
